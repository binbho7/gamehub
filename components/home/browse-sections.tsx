...
export function PlatformBrowse({ games }: { games: PublishedGame[] }) { return <div className="grid gap-3 md:grid-cols-3">{platforms.map(({ name, slug, icon: Icon }) => { const count = games.filter((game) => game.platforms.includes(name)).length; return <Link key={slug} href={`/platforms/${slug}`} className="group flex min-h-36 flex-col justify-between rounded-xl border bg-card p-5 transition hover:border-primary/50"><Icon className="size-6 text-primary" /><div><h3 className="font-semibold">{name}</h3><p className="mt-1 text-sm text-muted-foreground">{count} 款游戏</p></div></Link>; })}</div>; }
