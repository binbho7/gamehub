...
export function SearchDialog({ games }: { games: SearchGameRecord[] }) {
  ...
  return <Dialog.Root open={open} onOpenChange={(nextOpen) => { setOpen(nextOpen); if (!nextOpen) setQuery(""); }}>
    ...
    <Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm" /><Dialog.Content className="fixed left-1/2 top-[12vh] z-50 w-[calc(100%-2rem)] max-w-2xl -translate-x-1/2 rounded-xl border bg-[#0e131d] p-4 shadow-2xl sm:p-6">
      ...
      <div className="relative mt-4"><Search className="absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} className="pl-10" placeholder="搜索游戏名称、中文名、开发商或出版商" /></div>
      <div className="mt-4 flex max-h-[52vh] flex-col gap-1 overflow-y-auto">{results.map((game) => <Dialog.Close asChild key={game.slug}><Link href={`/games/${game.slug}`} className="flex items-center gap-3 rounded-lg p-2.5 hover:bg-secondary"><Image src={game.cover} width={48} height={64} alt="" className="h-16 w-12 rounded-md object-cover" /><div className="min-w-0"><p className="truncate font-medium">{game.title}</p><p className="truncate text-sm text-muted-foreground">{game.developer} · {game.releaseDate.slice(0,4)}</p></div></Link></Dialog.Close>)}{query && results.length === 0 && <p className="py-8 text-center text-sm text-muted-foreground">没有找到“{query}”，请尝试英文名称、开发商或出版商。</p>}</div>
