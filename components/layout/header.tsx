...
export async function Header() {
  ...
  const platforms = games.flatMap((game) => game.platforms.map((name, index) => ({ name, slug: game.platformSlugs?.[index] })).filter((item): item is { name: string; slug: string } => Boolean(item.slug)));
