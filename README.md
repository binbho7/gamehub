...
# GameHub V2.9 production data workflow

The production website is built from the tracked `generated/site-data.json` artifact. The artifact is produced by a deterministic, local-only D1 export with an operator-supplied `YYYY-MM-DD` snapshot date; it never derives a date from the wall clock. The reviewed bootstrap artifact uses `snapshotDate` `2026-09-19` and contains one eligible game.

The canonical Cloudflare Pages command is `npm run build`, which runs `npm run site:data:check` before `next build` and writes the static site to `out/`. A clean checkout needs only repository contents and installed dependencies: the build does not use local D1, `.wrangler`, credentials, network/provider APIs, or runtime APIs. The deployed static site has no runtime D1, R2, Worker, Container, Cron, or provider calls.

Publication data is fail-closed: production routes never fall back to `lib/mock-data.ts`; mock data is fixture-only. Public images must be approved Steam or IGDB remote sources, official links must be verified with method `manual`, `http`, or `provider_api`, and only valid YouTube videos enter the public DTO. V2.9 does not use R2 publication data. The artifact is limited to 10 MiB and 10,000 games.

Use `npm run site:data:check` to validate the tracked artifact before review. Do not commit operator reports or secrets, and do not automatically push or deploy from the export workflow.
