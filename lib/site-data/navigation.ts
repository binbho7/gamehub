...
export function buildTaxonomyNavigation(platforms: TaxonomyRecord[], genres: TaxonomyRecord[]): { platforms: TaxonomyLink[]; genres: TaxonomyLink[]; desktop: TaxonomyLink[]; mobile: TaxonomyLink[] } {
  ...
  const platformLinks = uniquePlatforms.slice(0, 3).map((item) => ({ label: item.name === "Windows" ? "PC" : item.name, href: `/platforms/${item.slug}` }));
  const genreLinks = uniqueGenres.slice(0, 3).map((item) => ({ label: item.name, href: `/genres/${item.slug}` }));
