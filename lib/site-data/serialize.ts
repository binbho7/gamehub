...
function normalizeGame(game: PublishedGame): PublishedGame {
  return {
    ...
    genres: game.genreSlugs ? game.genreSlugs.map((slug, index) => ({ slug, name: game.genres[index]! })).sort((a, b) => compare(a.name, b.name) || compare(a.slug, b.slug)).map((item) => item.name) : [...game.genres].sort(compare),
    platforms: game.platformSlugs ? game.platformSlugs.map((slug, index) => ({ slug, name: game.platforms[index]! })).sort((a, b) => compare(a.name, b.name) || compare(a.slug, b.slug)).map((item) => item.name) : [...game.platforms].sort(compare),
    ...(game.genreSlugs ? { genreSlugs: game.genreSlugs.map((slug, index) => ({ slug, name: game.genres[index]! })).sort((a, b) => compare(a.name, b.name) || compare(a.slug, b.slug)).map((item) => item.slug) } : {}),
    ...(game.platformSlugs ? { platformSlugs: game.platformSlugs.map((slug, index) => ({ slug, name: game.platforms[index]! })).sort((a, b) => compare(a.name, b.name) || compare(a.slug, b.slug)).map((item) => item.slug) } : {}),
    cover: game.cover,
    hero: game.hero,
