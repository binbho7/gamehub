...
describe("image ingest service", () => {
  it("ingests a fresh existing row with R2-first then optimistic D1 binding", async () => {
    ...
    expect(result).toEqual({ gameId: 10, status: "completed", preflightError: null, images: [{ imageId: 11, outcome: "ingested" }] });
  ...
  it("does a metadata HEAD only for an already-ingested row", async () => {
    ...
    const bucket = r2({ head: vi.fn(async () => ({ exists: true as const, size: 3, hash: HASH, sha256Metadata: HASH, mimeType: "image/jpeg", cacheControl: "public, max-age=31536000, immutable" })) });
    const repo = repository(snapshot([complete]));
    const result = await createImageIngestService(deps(repo, bucket)).ingest(10, { write: true });
    expect(result.images).toEqual([{ imageId: 11, outcome: "already_ingested" }]);
  ...
  it("dry-run performs real processing and R2 HEAD but no PUT or D1 writes", async () => {
    ...
    expect(result.images).toEqual([{ imageId: 11, outcome: "ingested" }]);
  ...
  it("returns game-level failure before any network or mutation above the image limit", async () => {
    ...
    const bucket = r2();
    const result = await createImageIngestService(deps(repo, bucket)).ingest(10, { write: true });
    expect(result).toEqual({ gameId: 10, status: "failed", preflightError: "image_limit_exceeded", images: [] });
